...
|---|---|
| 输入 | 📝 草稿输入 |
| 工具 | ✂️ 稿件精简（可选 1/4、1/3、1/2 篇幅）· 🎨 风格提取 · 📄 PDF 版面提取 · **🎙️ 录音转文字** · 💡 新闻选题策划 · 📋 报道方案生成 |
| AI 改写 | 📺 电视口播稿 · 📰 报刊通稿 |
...
- **右侧面板可拖拽调宽**（默认 520px，最宽 1100px，宽度本地记忆）；**输出/成稿预览为大窗口**（约 52vh 高、可上下拉伸），长稿不必在窄框里滚动
...
- **AI 服务后台可配**：系统设置里填写/更换 AI Token（带掩码显示与「清除回落 .env」）、选择默认模型档位、填写服务地址，并提供 **🔌 测试连接** 自检；**保存即生效，无需重启服务**
...
- **录音转文字**：上传采访录音（mp3/m4a/wav/amr 等，≤200MB）→ 本机语音识别转成文字稿 → 直接进改写流水线。**音频不出内网**、不需要 GPU、不计 AI 费用；10~60 分钟长录音走异步任务，进度实时可见，约 **7 分钟出 1 小时的录音**（实测字错率 3.7%）。服务端见 `deploy/asr-service/`
