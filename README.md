...
全站按一份 35 页的 UI 设计规范落地，**组件只引用设计令牌**（`frontend/src/tokens.css`），不写死色值：

- **三级纵深**：画布 `#070B12`（全站最深）→ 面板 `#0F1620` → 节点 `#141D29`，纵深由亮度差建立而非堆投影
- **连线与节点描边分色**（`#566D92` / `#4A5E80`）+ 箭头 + 曲率 0.45，让"谁连到谁"一眼可读；选中节点时非上下游连线降到 35%
- **节点 208×96 / 圆角 12 / 左侧 3px 分类色条 / 状态胶囊（颜色+图标+文字）**，五态可辨：待执行、运行中（呼吸 + 渐变进度条）、成功、失败（含错误摘要）、**选中（四角 L 形角标）**
- **图标全部为 Lucide 线性图标**（1.75px 描边、随分类色着色），全站零 emoji
- 顶栏 IA：品牌渐变 + 上下文 + 「运行工作流」主按钮 + 停止 + 运行状态；删除等危险操作移出顶栏
- 底部状态栏：缩放、节点数、连线数、最近执行时间与耗时（等宽数字）
- 动效令牌化，并响应 `prefers-reduced-motion`
...
| AI 改写 | 📺 电视口播稿 · 📰 报刊通稿 |
| 审稿 | ✅ 人工审定（**可直接修改稿件 → 保存为人工修订版 → 通过，下游即用改后版本**；也可批注/打回）· 🤖 AI 审稿（自动判定 + 审稿意见，可开严格模式） |
| 新媒体转换 | 💬 公众号长文 · 🔥 微博 · 🎬 抖音 · 📕 小红书 · 🗞️ 头条 |
| 成稿导出 | 5 平台排版成稿：公众号输出**内联样式 HTML**，一键复制富文本可直接粘贴进秀米 / 微信编辑器 |

- **取稿规则（执行与界面一致）**：先取直接上游节点的稿件；若中间夹了「人工审定 / AI 审稿」这类只审批不产稿的节点，会自动沿连线**逐级回溯**到最近的产稿节点 —— 所以「草稿 → 改写 → 人工审定 → 成稿导出」这种不带转换节点的链路也能直接出终稿
